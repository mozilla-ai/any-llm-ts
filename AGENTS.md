# Implementation Spec: rename-auth-header-anyllm-key
...
Run ONLY the tests related to your changes. Check package.json for the test runner (jest/vitest) and pass the relevant test file paths. Do NOT run the full test suite during development -- it is slow and the full suite runs automatically in CI. Lint: npx biome check . or the lint script in package.json.
